...
const app = firebase.initializeApp({
  apiKey: "AIzaSyBpa3GED1TZLQLhDHPGCIlF5OVNiOUYm2Q",
  authDomain: "pp-telebot.firebaseapp.com",
  databaseURL: "https://pp-telebot-default-rtdb.firebaseio.com",
  projectId: "pp-telebot",
  storageBucket: "pp-telebot.appspot.com",
  messagingSenderId: "14358957639",
});
const ref = firebase.database().ref("TaskListRecords");
chatIdRef = ref.child("chatIds");
...
function newStress(telegramChatId, newTask) {
  ...
  //     .ref("TaskListRecords")
  //     .orderByKey()
  //     .equalTo(telegramChatId.toString())
  //   firebase
  //     .database()
  //     .ref("TaskListRecords")
  //     .orderByChild(chatIdRef)
  //     .equalsTo(telegramChatId)
  ref
    .orderByChild("chatId")
    .equalTo(telegramChatId)
    .on("child_added", (snapshot) => {
      if (snapshot.exists()) {
        ...
        console.log("SNAPSHOT: " + tasklist);
        // tasklist = _.toArray(snapshot.val()).push(newTask);
        // console.log("LIST: " + tasklist);
        console.log("SNAP KEY: " + snapshot.key);
      } else {
        console.log("does not exist");
        tasklist.push(newTask);
        //telegram chat id is set as key
        ref.child(telegramChatId.toString()).set({
  ...
  //     //   console.log(snapshot.val());
  //     //   tasklist.push(newTask);
  //     console.log("exist, " + snapshot.key);
  // var currChatIdRef = chatIdRef.child(telegramChatId);
  // currChatIdRef.update({
  //   tasklist: taskList,
  // });
  // return;
  //     } else {
